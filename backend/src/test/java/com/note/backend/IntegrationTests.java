...
public class IntegrationTests {
    ...
    @Test
    public void watchedFlow() throws Exception {
        AuthReq reg = new AuthReq(); reg.username = "watchtest"; reg.password = "pass";
        mockMvc.perform(post("/api/auth/register")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(reg)))
                .andExpect(status().isOk());
        ...
        MovieReq m = new MovieReq(); m.movieId = 77777L; m.title = "Y"; m.posterPath = "/p.jpg"; m.overview = "o"; m.releaseDate = "2025-02-02";
        mockMvc.perform(post("/api/movies/watched")
                .header("Authorization", "Bearer " + token)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(m)))
                .andExpect(status().isOk());

        mockMvc.perform(get("/api/movies/watched").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(77777));

        mockMvc.perform(delete("/api/movies/watched/77777").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk());
    }
}
